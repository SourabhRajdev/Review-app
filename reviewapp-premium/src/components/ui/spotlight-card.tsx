...
// ── spotlightHandlers ─────────────────────────────────────────────────────────
// Spread onto any motion/HTML element. Sets --sx, --sy (pointer coords relative
// to the element) and --sl-op (0/1) so CSS radial-gradient overlays can track
// the pointer without a global listener.
export function spotlightHandlers() {
  return {
    ...
    onPointerEnter: (e: React.PointerEvent) => {
      (e.currentTarget as HTMLElement).style.setProperty('--sl-op', '1');
    },
    onPointerLeave: (e: React.PointerEvent) => {
      (e.currentTarget as HTMLElement).style.setProperty('--sl-op', '0');
    },
  };
}

// ── SpotlightOverlay ──────────────────────────────────────────────────────────
// Drop inside any `position: relative; overflow: hidden` container that has
// spotlightHandlers() attached. Renders the radial highlight.
export function SpotlightOverlay({
  color = 'warm',
  size = 120,
}: {
  color?: 'white' | 'warm' | 'coffee';
  size?: number;
}) {
  const fill =
    color === 'white'  ? 'rgba(255,255,255,0.30)' :
    color === 'coffee' ? 'rgba(198,124,78,0.13)'  :
                         'rgba(255,215,168,0.24)';
  ...
  return (
    <div
      ...
      style={{
        background: `radial-gradient(${size}px ${size}px at var(--sx,50%) var(--sy,50%), ${fill} 0%, transparent 80%)`,
        opacity: 'var(--sl-op,0)',
...
// ── SpotlightCard ─────────────────────────────────────────────────────────────
// Wraps any card content and adds the warm spotlight effect.
interface SpotlightCardProps {
  children: ReactNode;
  className?: string;
  style?: React.CSSProperties;
  color?: 'warm' | 'coffee' | 'white';
  size?: number;
}
...
export function SpotlightCard({
  ...
  return (
    <div
      ...
      style={style}
    >
      <SpotlightOverlay color={color} size={size} />
      {children}
    </div>
  );
}
...
// ── GlowCard ─────────────────────────────────────────────────────────────────
// Original spec: global pointer-tracking spotlight with glowing border effect.
